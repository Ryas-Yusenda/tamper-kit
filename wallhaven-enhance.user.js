...
(function () {
  ...
  class Pic {
    constructor(elem, ws) {
      ...
      this.picUrl = `https://w.wallhaven.cc/full/${f}/wallhaven-${this.id}.jpg`;
    ...
    addView() {
      ...
      $(this.elem)
        .find('.thumb-info')
        .append(
          $(`<a class="btn-view" style="margin-left:10px" target="_blank" href="${hrefView}"><i class="fa fa-fw fa-eye"></i></a>`).on('click', e => {
            e.stopPropagation();
  ...
  class WallhavenScript {
    ...
    workListMain() {
      const pics = this.getPics(),
        newPics = this.filterNewPics(pics);
  ...
  const callFancyBox = () => {
    loadFancybox().then(() => {
      ...
      $("[data-fancybox='gallery']").fancybox({ buttons: ['zoom', 'share', 'slideShow', 'fullScreen', 'download', 'thumbs', 'close'], thumbs: { autoStart: true } });
    });
  };
  ...
  // Load Lightgallery at the beginning
  loadLightGallery();
})();
